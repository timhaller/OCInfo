			...
			switch(lancer) {
				...
				case "panier":
					for (let i = 0; i < 2; i++) {
						...
						let fruit = Object.keys(this.arbres).filter(key => this.arbres[key] === max)[Math.floor(Math.random() * Object.keys(this.arbres).filter(key => this.arbres[key] === max).length)];
						this.panier[fruit] += 1;
						this.arbres[fruit] -= 1;
					}
					break;
...
document.addEventListener("DOMContentLoaded", async function () {
	...
	await simulate(gamesToSimulate).then((wins) => {
		console.log(wins)
		let msg = `Le corbeau a une chance de ${(wins["corbeau"]/ gamesToSimulate * 100).toFixed(2)}% de gagner`
