  ...
  async function processCommand(command) {
    ...
    switch (command[0]){
      ...
      case "ls":
        ...
        let module = checkPath(files, dirToList)? files.find(element => element.name === dirToList).exercices.map(element => element.name) : files.map(element => element.name)
        console.log(module);
      ...
      case "cd":
        ...
        console.log(dirToGo);
        checkPath(files, dirToGo) || dirToGo === "~"? (path = dirToGo, pathField.innerHTML = `${path}`) : writeToTerminal(`Directory not found: ${dirToGo}`);
        break;
      case "clear":
      ...
      case "open":
        ...
        if (checkPath(files, basePath)) {
          exercice = files.find(element => element.name === basePath).exercices.find(exercice => exercice.name === fileToOpen)
        } else {
