...
function loop() {
    animate ? document.getElementById("card-letters").innerHTML = randomString(1500): null
...
function newlines(amount) {
    ...
    for (let i = 0; i < amount; i++) {
        let newline = `<span style='overflow: hidden; word-wrap: normal; color: ${colors[Math.floor(Math.random()*colors.length)]}'>` + randomString(100) + "</span> ";
        str += newline;
    }
    return str;
}
...
function mute() {
    ...
    document.getElementById("audio-icon").innerHTML = muted ? mutedIcon : audioIcon;
}
    ...
    letters.onmouseenter = () => {
        ...
        letters.innerHTML = `
            ...
            <span style="width: fit-content; background: darkgreen; word-wrap: normal">
                ${randomString(30)}<span>&nbsp;<a href='exercices.html' style='color: white; display: inline'>Exercices</a>&nbsp;</span>${randomString(30)}
            </span>
